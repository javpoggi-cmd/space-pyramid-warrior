document.addEventListener('DOMContentLoaded', () => {
    ...
    const assetPaths = { player: 'img/player.png', drone: 'img/drone.png', droneBullet: 'img/drone_bullet.png', bullet: 'img/bullet.png', heavyBullet: 'img/heavy_bullet.png', missile: 'img/missile.png', enemyBullet: 'img/enemy_bullet.png', explosion1: 'img/explosion1.png', explosion2: 'img/explosion2.png', explosion3: 'img/explosion3.png', shieldEffect1: 'img/shield_effect_1.png', shieldEffect2: 'img/shield_effect_2.png', shieldEffect3: 'img/shield_effect_3.png', enemy1: 'img/enemy1.png', enemy2: 'img/enemy2.png', enemy3: 'img/enemy3.png', enemy4: 'img/enemy4.png', enemy5: 'img/enemy5.png', enemy6: 'img/enemy6.png', enemy7: 'img/enemy7.png', superBoss1: 'img/super_boss_1.png', superBoss2: 'img/super_boss_2.png', superBoss3: 'img/super_boss_3.png', superBoss4: 'img/super_boss_4.png', gigaBoss: 'img/giga_boss.png', finalEnemy: 'img/final_enemy.png', asteroid1: 'img/asteroid1.png', asteroid2: 'img/asteroid2.png', powerupShield: 'img/powerup_shield.png', powerupRapidFire: 'img/powerup_rapidfire.png', powerupExtraLife: 'img/powerup_extralife.png', powerupWings: 'img/powerup_wings.png', powerupHeavy: 'img/powerup_heavy.png', powerupMissile: 'img/powerup_missile.png', powerupBomb: 'img/powerup_bomb.png', powerupLuck: 'img/powerup_luck.png', powerupDrone: 'img/powerup_drone.png', missileIcon: 'img/missile_icon.png', homingBullet: 'img/homing_bullet.png', introScreen: 'img/intro_screen.png', ending1: 'img/ending_1.png', ending2: 'img/ending_2.png', ending3: 'img/ending_3.png', ending4: 'img/ending_4.png', ending5: 'img/ending_5.png' };
    const audioPaths = { backgroundMusic: 'audio/background_music.mp3', bossMusic: 'audio/boss_music.mp3', introMusic: 'audio/intro_music.mp3', endingMusic: 'audio/ending_music.mp3', playerShoot: 'audio/player_shoot.wav', heavyShoot: 'audio/heavy_shoot.wav', missileLaunch: 'audio/missile_launch.wav', missileExplosion: 'audio/missile_explosion.wav', enemyShoot: 'audio/enemy_shoot.wav', explosionSmall: 'audio/explosion_small.wav', explosionLarge: 'audio/explosion_large.wav', bossExplosion: 'audio/boss_explosion.wav', powerupShield: 'audio/powerup_shield.wav', powerupBurst: 'audio/powerup_burst.wav', powerupExtraLife: 'audio/powerup_extralife.wav', powerupWings: 'audio/powerup_wings.wav', powerupHeavy: 'audio/powerup_heavy.wav', powerupMissile: 'audio/powerup_missile.wav', powerupBombPickup: 'audio/powerup_bomb_pickup.wav', powerupLuck: 'audio/powerup_luck.wav', powerupDrone: 'audio/powerup_drone.wav', bombExplode: 'audio/bomb_explode.wav', hit: 'audio/hit.wav', playerDamaged: 'audio/player_damaged.wav' };
    ...
    class PowerUp { constructor(x, y, type) { this.x = x; this.y = y; this.type = type; this.width = 75 * scaleFactor; this.height = 75 * scaleFactor; this.speed = 2.5 * scaleFactor; let imgKey = 'powerupShield'; if (type === 'rapidFire') imgKey = 'powerupRapidFire'; if (type === 'extraLife') imgKey = 'powerupExtraLife'; if (type === 'wingCannons') imgKey = 'powerupWings'; if (type === 'heavyCannon') imgKey = 'powerupHeavy'; if (type === 'missileSystem') imgKey = 'powerupMissile'; if (type === 'smartBomb') imgKey = 'powerupBomb'; if (type === 'luckUp') imgKey = 'powerupLuck'; if (type === 'drone') imgKey = 'powerupDrone'; this.image = assets[imgKey]; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.y += this.speed; } }
    ...
    switch (this.type) {
        case 1:
            ...
            break;
        case 3:
            enemyBullets.push(new EnemyBullet(this.x + this.width * 0.2, bulletY));
            enemyBullets.push(new EnemyBullet(this.x + this.width * 0.8, bulletY));
            break;
        case 4:
            const speedX = 2 * this.zigzagDir;
            const speedY = 5;
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, speedX - 1.5, speedY));
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, speedX - 0.5, speedY));
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, speedX + 0.5, speedY));
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, speedX + 1.5, speedY));
            break;
        case 5:
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, 0, 7));
            enemyBullets.push(new EnemyBullet(bulletX, this.y, 0, -7));
            break;
        case 6: 
            
            enemyBullets.push(new EnemyBullet(bulletX, bulletY, -2, 6)); // speedX, speedY
        ...
        case 7: 
            ...
            setTimeout(() => {
                ...
                const speedX_right = Math.cos(angleRad) * bulletSpeed;
                const speedY_up = Math.sin(angleRad) * bulletSpeed;
                enemyBullets.push(new EnemyBullet(this.x + this.width / 2, this.y, speedX_right, speedY_up));

                const speedX_left = -speedX_right;
                enemyBullets.push(new EnemyBullet(this.x + this.width / 2, this.y, speedX_left, speedY_up));
            }, 500);
            break;
        default:
    ...
    [difficultyTimer, asteroidInterval, bossTimer, meteorShowerTimer, aggressiveAsteroidSpawner, missileChargeInterval, luckUpTimeout, waveTimer].forEach(timer => {
    ...
    initGame(currentBossIndex); 
}
    function togglePause() {
    isPaused = !isPaused;

    if (isPaused) {
        // Detiene el juego y la música
        cancelAnimationFrame(animationFrameId);
        if (audioAssets.backgroundMusic) audioAssets.backgroundMusic.pause();
        if (audioAssets.bossMusic) audioAssets.bossMusic.pause();

        // Crea el contenedor del menú de pausa
        ...
        // --- HTML del nuevo menú de pausa ---
        pauseMenu.innerHTML = `
            <h1 style="font-size: 2.5em; color: #00ff00; text-shadow: 2px 2px 4px #000; margin-top: 0;">PAUSA</h1>
            
            <div id="mainPauseButtons" style="margin-bottom: 25px;">
                <button id="resumeButton" style="padding: 12px 24px; font-size: 1.2em; margin: 5px; cursor: pointer;">Seguir</button>
                <button id="restartButton" style="padding: 12px 24px; font-size: 1.2em; margin: 5px; cursor: pointer;">Reiniciar Nivel</button>
                <button id="exitButton" style="padding: 12px 24px; font-size: 1.2em; margin: 5px; cursor: pointer;">Salir</button>
            </div>

            <div id="pauseSoundControls" style="border-top: 1px solid #444; padding-top: 15px;">
                <div style="display: flex; justify-content: space-around; align-items: center; margin-bottom: 10px;">
                    <button id="musicToggleButton" style="flex-basis: 45%; padding: 10px;">Música</button>
                    <button id="sfxToggleButton" style="flex-basis: 45%; padding: 10px;">SFX</button>
                </div>
                <div style="margin-bottom: 10px;">
                    <label for="musicVolumeSlider">Volumen Música</label>
                    <input type="range" id="musicVolumeSlider" min="0" max="1" step="0.05" style="width: 90%;">
                </div>
                <div>
                    <label for="sfxVolumeSlider">Volumen SFX</label>
                    <input type="range" id="sfxVolumeSlider" min="0" max="1" step="0.05" style="width: 90%;">
                </div>
            </div>
        `;
        gameContainer.appendChild(pauseMenu);

        // --- Lógica de los nuevos controles ---
        ...
        restartButton.onclick = restartCurrentLevel; // <-- Usa la nueva función
        exitButton.onclick = exitToLobby;

        // Lógica para botones ON/OFF
        const updateMusicButtonText = () => { musicToggleButton.textContent = `Música: ${isMusicOn ? 'ON' : 'OFF'}`; };
        const updateSfxButtonText = () => { sfxToggleButton.textContent = `SFX: ${isSfxOn ? 'ON' : 'OFF'}`; };
        ...
        musicToggleButton.onclick = () => {
            ...
            updateMusicButtonText();
            if (!isMusicOn) {
                playMusic(null); // Detiene la música si se apaga
            }
        };
        sfxToggleButton.onclick = () => {
            isSfxOn = !isSfxOn;
            updateSfxButtonText();
        };

        // Lógica para sliders de volumen
        musicVolumeSlider.value = musicVolume;
        sfxVolumeSlider.value = sfxVolume;

        musicVolumeSlider.addEventListener('input', (e) => {
            musicVolume = parseFloat(e.target.value);
            // Actualiza el volumen de la música que debería estar sonando
    ...
    } else { // --- Lógica cuando se reanuda el juego ---
        // Reanuda la música correcta si está encendida
        if (isMusicOn) {
            const currentTrack = (gameState === 'NORMAL_WAVE') ? audioAssets.backgroundMusic : audioAssets.bossMusic;
    ...
    for (let i = 0; i < 20; i++) {
        setTimeout(() => {
            ...
            if (i % 2 === 0) {
                playSound(audioAssets.explosionLarge, 0.7);
            }
    ...
    let introStartTime; async function initIntro() { appState = 'INTRO'; startScreen.style.display = 'none'; canvas.style.display = 'block'; await Promise.all([preloadAssets(), preloadAudio()]); if (isTouchDevice()) setupTouchControls(); introStartTime = Date.now(); playMusic(audioAssets.introMusic); introLoop(); }
