document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Clases del Juego ---
    class Player { constructor() { this.image = assets.player; this.width = GAME_CONFIG.player.width; this.height = GAME_CONFIG.player.height; this.x = canvas.width / 2 - this.width / 2; this.y = canvas.height + this.height; this.speed = GAME_CONFIG.player.speed; this.isPositioned = false; } draw() { if (this.image) { if (isInvulnerable && !shieldStacks) { ctx.globalAlpha = (Math.floor(Date.now() / 100) % 2 === 0) ? 0.5 : 1; } ctx.drawImage(this.image, this.x, this.y, this.width, this.height); ctx.globalAlpha = 1; if (shieldStacks > 0 && assets[`shieldEffect${shieldStacks}`]) { const shieldImage = assets[`shieldEffect${shieldStacks}`]; ctx.drawImage(shieldImage, this.x - 20, this.y - 20, this.width + 40, this.height + 40); } } } update() { if (!this.isPositioned) { if (this.y > canvas.height - 150) { this.y -= 2; } else { this.isPositioned = true; } return; } let moveX = 0; let moveY = 0; if (keys['arrowleft'] || keys['a']) moveX -= 1; if (keys['arrowright'] || keys['d']) moveX += 1; if (keys['arrowup'] || keys['w']) moveY -= 1; if (keys['arrowdown'] || keys['s']) moveY += 1; const gamepad = navigator.getGamepads()[0]; if (gamepad) { const stickX = gamepad.axes[0]; const stickY = gamepad.axes[1]; const deadzone = 0.2; if (Math.abs(stickX) > deadzone) moveX = stickX; if (Math.abs(stickY) > deadzone) moveY = stickY; if (gamepad.buttons[0].pressed) { this.shoot(); } } if (touchMoveX) moveX = touchMoveX; if (touchMoveY) moveY = touchMoveY; if (moveX !== 0) this.x += this.speed * moveX; if (moveY !== 0) this.y += this.speed * moveY; if (this.x < 0) this.x = 0; if (this.x > canvas.width - this.width) this.x = canvas.width - this.width; if (this.y < 0) this.y = 0; if (this.y > canvas.height - this.height) this.y = canvas.height - this.height; } shoot() { const now = Date.now(); const currentCooldown = (heavyCannonLevel > 0) ? GAME_CONFIG.player.heavyCannonCooldown : GAME_CONFIG.player.shootCooldown; if (!this.isPositioned || now - playerLastShotTime < currentCooldown) return; if (homingSidekickActive) { const target = findNearestTarget(this.x, this.y, ['enemies', 'bosses']); if (target) { bullets.push(new SidekickBullet(this.x + this.width / 2 - 5, this.y, target)); } } if (heavyCannonLevel > 0) { playSound(audioAssets.heavyShoot, 0.8); const shots = heavyCannonLevel; for (let i = 0; i < shots; i++) { setTimeout(() => { if (gameRunning) bullets.push(new HeavyBullet(this.x + this.width / 2 - 15, this.y)); }, i * 120); } } else { playSound(audioAssets.playerShoot, 0.7); let shotsPerBurst = 1; if (burstFireLevel === 1) shotsPerBurst = 2; if (burstFireLevel === 2) shotsPerBurst = 3; const fire = (xOffset) => { for (let i = 0; i < shotsPerBurst; i++) { setTimeout(() => { if (gameRunning) bullets.push(new Bullet(this.x + xOffset, this.y)); }, i * 100); } }; if (wingCannonsActive) { fire(this.width * 0.2); fire(this.width * 0.8 - 10); } else { fire(this.width / 2 - 5); } } playerLastShotTime = now; } }
    class Bullet { constructor(x, y) { this.image = assets.bullet; this.x = x; this.y = y; this.width = 10; this.height = 30; this.speed = 12; this.damage = 1; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.y -= this.speed; } }
    ...
    class DroneBullet {
        constructor(x, y, target) {
            ...
            this.width = 15;
            this.height = 15;
            this.speed = GAME_CONFIG.drones.bulletSpeed;
            this.damage = 0.6;
        }
        draw() {
            if (this.image) {
    ...
    class Drone {
        constructor(player, angleOffset) {
            ...
            this.lastShotTime = 0;
            this.target = null; // El dron ahora recuerda su objetivo
        }

        update() {
            this.angle += GAME_CONFIG.drones.rotationSpeed;
            this.x = this.player.x + this.player.width / 2 + Math.cos(this.angle) * GAME_CONFIG.drones.orbitRadius - this.width / 2;
            this.y = this.player.y + this.player.height / 2 + Math.sin(this.angle) * GAME_CONFIG.drones.orbitRadius - this.height / 2;
    ...
    class HeavyBullet { constructor(x, y) { this.image = assets.heavyBullet; this.x = x; this.y = y; this.width = 30; this.height = 50; this.speed = 8; this.damage = 15; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.y -= this.speed; } }
    class SidekickBullet { constructor(x, y, target) { this.image = assets.sidekickBullet; this.x = x; this.y = y; this.target = target; this.width = 12; this.height = 12; this.speed = 10; this.damage = 0.5; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { if (this.target && this.target.health > 0) { const angle = Math.atan2(this.target.y - this.y, this.target.x - this.x); this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; } else { this.y -= this.speed; } } }
    class Missile { constructor(x, y, target) { this.image = assets.missile; this.x = x; this.y = y; this.target = target; this.width = 40; this.height = 80; this.speed = 4; this.damage = 15; } draw() { if (this.image) { ctx.save(); ctx.translate(this.x + this.width / 2, this.y + this.height / 2); const angle = this.target ? Math.atan2(this.target.y - this.y, this.target.x - this.x) + Math.PI / 2 : 0; ctx.rotate(angle); ctx.drawImage(this.image, -this.width / 2, -this.height / 2, this.width, this.height); ctx.restore(); } } update() { if (this.target && this.target.health > 0) { const angle = Math.atan2(this.target.y + this.target.height / 2 - this.y, this.target.x + this.target.width / 2 - this.x); this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; } else { this.y -= this.speed; } } }
    class EnemyBullet { constructor(x, y, speedX = 0, speedY = 7) { this.image = assets.enemyBullet; this.x = x; this.y = y; this.width = 32; this.height = 32; this.speedX = speedX; this.speedY = speedY; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.x += this.speedX; this.y += this.speedY; } }
    class HomingEnemyBullet { constructor(x, y, target) { this.image = assets.enemyBullet; this.x = x; this.y = y; this.target = target; this.width = 40; this.height = 40; this.speed = 3; } draw() { if (this.image) { ctx.filter = 'hue-rotate(120deg) brightness(1.5)'; ctx.drawImage(this.image, this.x, this.y, this.width, this.height); ctx.filter = 'none'; } } update() { if (this.target && player) { const angle = Math.atan2(this.target.y - this.y, this.target.x - this.x); this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; } else { this.y += this.speed; } } }
    class MineBullet { constructor(x, y) { this.x = x; this.y = y; this.width = 25; this.height = 25; this.speedY = 1.5; this.life = 200; } draw() { ctx.fillStyle = `hsl(${this.life * 2}, 100%, 50%)`; ctx.fillRect(this.x, this.y, this.width, this.height); } update() { this.y += this.speedY; this.life--; if (this.life <= 0) { this.explode(); } } explode() { playSound(audioAssets.explosionSmall, 0.7); for (let i = 0; i < 8; i++) { const angle = (i / 8) * (Math.PI * 2); enemyBullets.push(new EnemyBullet(this.x, this.y, Math.cos(angle) * 4, Math.sin(angle) * 4)); } } }
    ...
    function initGame(startProgressionIndex = 0) {
        ...
        if (applyAllPowerupsCheat) {
            ...
            // drones.push(new Drone(player, Math.PI));
    ...
    function gameLoop() {
        ...
        ctx.fillText(`Vidas: ${playerLives}`, 20, 70);
        ctx.fillText(`Salud: ${playerHealth}`, 20, 100);
        ctx.textAlign = 'right';
        ctx.fillText(`Jefes: ${bossesDestroyed}`, canvas.width - 20, 40);
        
        if (missileSystemActive && assets.missileIcon) {
            const iconSize = 80;
            const totalWidth = GAME_CONFIG.missiles.maxCharges * (iconSize + 10);
            for (let i = 0; i < missileCharges; i++) {
                ctx.drawImage(assets.missileIcon, (canvas.width / 2) - (totalWidth / 2) + (i * (iconSize + 10)), canvas.height - (iconSize + 10), iconSize, iconSize);
            }
        }
        ...
        for (let pIndex = projectiles.length - 1; pIndex >= 0; pIndex--) {
            ...
            for (let tIndex = targets.length - 1; tIndex >= 0; tIndex--) {
                ...
                if (p.x < hitbox.x + hitbox.width && p.x + p.width > hitbox.x && p.y < hitbox.y + hitbox.height && p.y + p.height > hitbox.y) {
                    ...
                    explosions.push(new Explosion(p.x, p.y, p instanceof Missile ? 150 : 30));
    ...
    function handleTargetDestroyed(target) {
        if (target instanceof Boss) {
            ...
            if (enemyDestroyedCount > 0 && enemyDestroyedCount % 20 === 0) {
                ...
                // else if (rand < 0.78) type = 'sidekick';
                // else if (rand < 0.90) type = 'drone';
    ...
    function launchMissile() {
        if (player && gameRunning && !isPaused && missileCharges > 0) {
            ...
            if (target) {
                missiles.push(new Missile(player.x + player.width / 2 - 10, player.y, target));
                missileCharges--;
