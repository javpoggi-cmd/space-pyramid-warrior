document.addEventListener('DOMContentLoaded', () => {
    ...
    const GAME_CONFIG = {
        ...
        gameplay: {
            chargeCycleDuration: 20000, 
            superBossChargeCycleDuration: 90000,
            intermissionDuration: 2000, 
            maxEnemiesOnScreen: 45
        
        },
        chainLightning: {
            damage: 4,
            life: 55, // Aumentamos la vida para dar tiempo al segundo pulso
            maxTargets: 16,
            searchRadius: 250,
            pulseDelay: 60 // 30 frames = 500ms a 60fps
        }
    };
    ...
    let players = [];
    let bullets = [], enemyBullets = [], asteroids = [], enemies = [], explosions = [], smallExplosions = [], smokeEffects = [], spawnQueue = [], stars = [], powerUps = [], bosses = [], missiles = [], drones = [], laserBeams = [], asteroidShots = [], homingPowerUps = [], impactExplosions = [], chainLightnings = [];
    ...
    let gameOverDelayStartTime = null;
    let musicPlaylist = []; 
    ...
    let isHighScoreInputActive = false; // Nos dice si la pantalla de ingreso está activa
    let highScoreEntryData = []; // Guardará los datos de los jugadores que calificaron
    let currentInitials = []; // Guardará las iniciales que se están editando
    let activePlayerInputIndex = 0; // Qué jugador está editando (0 para P1, 1 para P2, etc.)
    let activeLetterInputIndex = 0; // Qué letra está seleccionada (0, 1, o 2)
...
// --- Variables de Sonido, Assets y Cheats ---
    ...
    const assetPaths = { 
        ...
        shieldEffect4: 'img/shield_effect_4.png', shieldEffect5: 'img/shield_effect_5.png', 
...
asteroid5_f: 'img/asteroid5_f.png',
        ...
        powerupLuck: 'img/powerup_luck.png',
    ...
    chainLightningEffect_1: 'img/chainLightningEffect_1.png',
    chainLightningEffect_2: 'img/chainLightningEffect_2.png',
    chainLightningEffect_3: 'img/chainLightningEffect_3.png',
    chainLightningEffect_4: 'img/chainLightningEffect_4.png',
    ...
    function playMusic(track) {
        Object.values(audioAssets).forEach(audio => {
            if (audio instanceof HTMLAudioElement && !audio.paused) {
                audio.pause();
                audio.currentTime = 0; // Opcional: rebobinar la pista
            }
        });
        if (isMusicOn && track) {
    ...
    function playSound(sound, volume = 1.0) { 
        if (isSfxOn && sound) { 
            ...
            // --- CORRECCIÓN ---
            // Usamos Math.min() para asegurar que el volumen nunca sea mayor a 1.
            soundInstance.volume = Math.min(1, sfxVolume * volume); 
...
class Player {
    constructor(id) {
        ...
        this.lastShotTime = 0;
        this.velocityX = 0; // Velocidad actual en el eje X
        this.velocityY = 0; // Velocidad actual en el eje Y
        this.magnetStacks = 0;
        this.isAuraActive = false;
        this.auraVisual = null;
        this.levelPowerUpStacks = 0;
        this.lastLevelShotTime = 0;
        this.hasChainLightningCharge = false;
        this.levelPowerShotCooldown = 10000;
        this.heavyCannonTimeoutId = null;
        this.laserTimeoutId = null;
    ...
    draw() {
        if (this.image) {
            if (this.flashTimer && this.flashTimer > 0) {
                ctx.save();
                ctx.globalCompositeOperation = 'lighter'; // Modo de brillo
                ctx.globalAlpha = (this.flashTimer / 15) * 0.8; // La opacidad disminuye con el tiempo (15 es la duración inicial)
                ctx.drawImage(this.image, this.x, this.y, this.width, this.height);
                ctx.restore(); // Restaura composite y alpha
                this.flashTimer--;
            }
    ...
    update() {
        ...
        let moveX = 0;
        let moveY = 0;
        // 1. Definimos las tasas de aceleración y desaceleración
        const acceleration = this.speed / 8; // Alcanza la velocidad máxima en 6 frames
        const deceleration = this.speed / 6; // Se detiene desde la máxima velocidad en 4 frames

        // 2. Recopilamos la intención de movimiento de todas las fuentes (teclado, gamepad, táctil)
        // Esta parte es igual que antes, solo determina la dirección deseada.
        ...
        // --- Controles Gamepad ---
        const gamepad = navigator.getGamepads()[this.id - 1];
        if (gamepad) {
        ...
        // 3. Aplicamos la aceleración o desaceleración a la velocidad actual
        // Eje X
        if (moveX !== 0) {
            this.velocityX += acceleration * moveX;
        } else {
            if (this.velocityX > 0) {
                this.velocityX = Math.max(0, this.velocityX - deceleration);
            } else if (this.velocityX < 0) {
                this.velocityX = Math.min(0, this.velocityX + deceleration);
            }
        }
        // Eje Y
        if (moveY !== 0) {
            this.velocityY += acceleration * moveY;
        } else {
            if (this.velocityY > 0) {
                this.velocityY = Math.max(0, this.velocityY - deceleration);
            } else if (this.velocityY < 0) {
                this.velocityY = Math.min(0, this.velocityY + deceleration);
            }
        }

        // 4. Limitamos la velocidad para que no supere la máxima
        ...
        // 5. Aplicamos la velocidad final a la posición de la nave
        this.x += this.velocityX;
        this.y += this.velocityY;

        // 6. Mantenemos a la nave dentro de los límites de la pantalla
    ...
    shoot() {
        ...
        if (this.wingCannonsLevel >= 1) {
            bullets.push(new LevelBullet(this.x + this.width * 0.2, this.y, this)); //... Se añade "this"
            bullets.push(new LevelBullet(this.x + this.width * 0.8 - (20 * scaleFactor), this.y, this)); //... Se añade "this"
        } else {
            bullets.push(new LevelBullet(this.x + this.width / 2 - (10 * scaleFactor), this.y, this)); //... Se añade "this"
        }
        ...
        if (this.laserLevel > 0) {
            ...
            laserBeams.push(new LaserBeam(this.x + this.width / 2, this.y, this)); //... Se añade "this"
            
            const createBurst = (angle) => {
                for (let i = 0; i < 2; i++) { // Ráfaga de 2 disparos
                    setTimeout(() => {
                        if (gameRunning) {
                            bullets.push(new Bullet(this.x + this.width * 0.1, this.y, this, -Math.PI / 2 - angle)); //... Se añade "this"
                            bullets.push(new Bullet(this.x + this.width * 0.9 - (10 * scaleFactor), this.y, this, -Math.PI / 2 + angle)); //... Se añade "this"
                        }
                    }, i * 100); // Pequeño retraso entre disparos de la ráfaga
                }
            };

            if (this.wingCannonsLevel === 2) {
                createBurst(0.2); // Ángulo para Nivel 2
            } else if (this.wingCannonsLevel >= 3) {
                createBurst(0.2); // Ángulo para Nivel 2
                createBurst(0.4); // Ángulo extra y más abierto para Nivel 3
            }
                ...
                setTimeout(() => {
                    if (gameRunning) {
                        ...
                        if (this.heavyCannonLevel === 1) {
                             bullets.push(new HeavyBullet(this.x + this.width / 2 - (15 * scaleFactor), this.y, this)); //... Se añade "this"
                        }
                        // Nivel 2 y 3: Dos tiros desde las alas
                        if (this.heavyCannonLevel >= 2 && this.heavyCannonLevel < 4) {
                             bullets.push(new HeavyBullet(this.x + this.width * 0.2, this.y, this)); //... Se añade "this"
                             bullets.push(new HeavyBullet(this.x + this.width * 0.8 - (30 * scaleFactor), this.y, this)); //... Se añade "this"
                        }
                        // Nivel 4 y 5: Cuatro tiros (dos rectos, dos en ángulo)
                        if (this.heavyCannonLevel >= 4) {
                            const angleSpread = 0.25;
                            bullets.push(new HeavyBullet(this.x + this.width * 0.1, this.y, this)); //... Se añade "this"
                            bullets.push(new HeavyBullet(this.x + this.width * 0.9 - (30 * scaleFactor), this.y, this)); //... Se añade "this"
                            bullets.push(new HeavyBullet(this.x + this.width * 0.1, this.y, this, -Math.PI / 2 - angleSpread)); //... Se añade "this"
                            bullets.push(new HeavyBullet(this.x + this.width * 0.9 - (30 * scaleFactor), this.y, this, -Math.PI / 2 + angleSpread)); //... Se añade "this"
                        }
                        if (this.wingCannonsLevel >= 2) {
                            const angleSpread = 0.4; // Ángulo abierto
                            // Añade disparos normales en ángulo, además de los pesados.
                            bullets.push(new Bullet(this.x + this.width * 0.1, this.y, this, -Math.PI / 2 - angleSpread)); //... Se añade "this"
                            bullets.push(new Bullet(this.x + this.width * 0.9 - (10 * scaleFactor), this.y, this, -Math.PI / 2 + angleSpread)); //... Se añade "this"
                        }

                        if (this.burstFireLevel > 0) {
                            // Después del disparo pesado, dispara una ráfaga de balas normales
                            for (let j = 0; j < this.burstFireLevel; j++) {
                                setTimeout(() => {
                                    if(gameRunning) {
                                        bullets.push(new Bullet(this.x + this.width / 2 - (5 * scaleFactor), this.y, this)); //... Se añade "this"
                                    }
                                }, (j + 1) * 100); // Dispara las balas normales después del disparo pesado
                            }
                        }
                    }
                }, i * 120);
            ...
            for (let i = 0; i < shotsPerBurst; i++) {
                setTimeout(() => {
                    if (gameRunning) {
                        ...
                        if (this.wingCannonsLevel >= 1) {
                            bullets.push(new Bullet(this.x + this.width * 0.2, this.y, this)); //... Se añade "this"
                            bullets.push(new Bullet(this.x + this.width * 0.8 - (10 * scaleFactor), this.y, this)); //... Se añade "this"
                        }
                        if (this.wingCannonsLevel >= 2) {
                            const angleSpread = 0.2;
                            bullets.push(new Bullet(this.x + this.width * 0.2, this.y, this, -Math.PI / 2 - angleSpread)); //... Se añade "this"
                            bullets.push(new Bullet(this.x + this.width * 0.8 - (10 * scaleFactor), this.y, this, -Math.PI / 2 + angleSpread)); //... Se añade "this"
                        }
    ...
    class Bullet { constructor(x, y, owner, angle = -Math.PI / 2) {
        this.owner = owner;
    ...
    draw() { 
        if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.x += this.speedX; this.y += this.speedY;

         } }
    class LaserBeam { constructor(x, y, owner) { this.owner = owner; this.image = assets.laser; this.x = x - (20 * scaleFactor / 2); this.y = y - canvas.height; this.width = 20 * scaleFactor; this.height = canvas.height; this.damage = 15; this.life = 60; this.collidedEnemies = new Set(); this.originY = y; } draw() { if (this.image) { ctx.globalAlpha = this.life / 60; ctx.drawImage(this.image, this.x, this.y, this.width, this.height); ctx.globalAlpha = 1; } } update() { this.life--; } }
    class AsteroidShot {
    ...
    class Missile { constructor(x, y, target, owner) { 
        this.owner = owner; 
        this.image = assets.missile; 
        this.x = x; this.y = y; 
        this.target = target; 
    ...
    draw() { 
        if (this.image) { ctx.save(); ctx.translate(this.x + this.width / 2, this.y + this.height / 2); const angle = this.target ? Math.atan2(this.target.y - this.y, this.target.x - this.x) + Math.PI / 2 : 0; ctx.rotate(angle); ctx.drawImage(this.image, -this.width / 2, -this.height / 2, this.width, this.height); ctx.restore(); } } update() { if (this.target && this.target.health > 0) { const angle = Math.atan2(this.target.y + this.target.height / 2 - this.y, this.target.x + this.target.width / 2 - this.x); this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; } else { this.y -= this.speed; } } 
    }
    class EnemyBullet { constructor(x, y, speedX = 0, speedY = 6) { this.image = assets.enemyBullet; this.x = x; this.y = y; this.width = 34 * scaleFactor; this.height = 34 * scaleFactor; this.speedX = speedX * scaleFactor; this.speedY = speedY * scaleFactor; } draw() { if (this.image) ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } update() { this.x += this.speedX; this.y += this.speedY; } }
...
class ChainLightning {
    constructor(player) {
        this.player = player;
        this.chains = [];
        // Leemos los parámetros desde GAME_CONFIG
        this.damage = GAME_CONFIG.chainLightning.damage;
        this.life = GAME_CONFIG.chainLightning.life;
        this.maxLife = this.life;
        this.maxTargets = GAME_CONFIG.chainLightning.maxTargets;
        this.pulseDelay = GAME_CONFIG.chainLightning.pulseDelay;

        this.player.flashTimer = 15;
        this.hasFiredSecondPulse = false;

        // --- NUEVO: Variables para la animación secuencial ---
        this.visibleChainCount = 0; // Cuántas cadenas son visibles actualmente
        this.chainRevealTimer = 0;
        this.chainRevealDelay = 2; // Tiempo en frames entre cada salto (muy rápido para efecto "zip")
        
        // La búsqueda de objetivos sigue siendo instantánea
        this.findAndChainTargets(this.player, [this.player]);
        // El primer pulso de daño también es instantáneo
        this.applyDamage();

        playSound(audioAssets.laserShoot, 1.5);
    }

    findAndChainTargets(source, hitTargets) {
        // ... Esta función no necesita cambios, la dejamos como está ...
        if (hitTargets.length >= this.maxTargets) return;
        const potentialTargets = [...enemies, ...bosses].filter(t => t.health > 0 && !hitTargets.includes(t));
        if (potentialTargets.length === 0) return;

        const searchRadiusSq = (GAME_CONFIG.chainLightning.searchRadius * scaleFactor) ** 2;
        let nearbyTargets = [];

        potentialTargets.forEach(target => {
            const sourceX = source === this.player ? source.x + source.width / 2 : source.x;
            const sourceY = source === this.player ? source.y : source.y;
            const dx = (target.x + target.width / 2) - sourceX;
            const dy = (target.y + target.height / 2) - sourceY;
            let distanceSq = dx * dx + dy * dy;

            if (target instanceof Boss) distanceSq *= 0.1;

            if (distanceSq < searchRadiusSq) {
                nearbyTargets.push({ target, distanceSq });
            }
        });

        nearbyTargets.sort((a, b) => a.distanceSq - b.distanceSq);
        const targetsToHit = nearbyTargets.slice(0, 2);

        targetsToHit.forEach(item => {
            const newTarget = item.target;
            if (!hitTargets.includes(newTarget)) {
                this.chains.push([source, newTarget]);
                this.findAndChainTargets(newTarget, [...hitTargets, newTarget]);
            }
        });
    }
    ...
    applyDamage() {
        ...
        this.chains.forEach(([source, target]) => {
            if (target !== this.player && !damagedEnemies.has(target)) {
                target.health -= this.damage;
                target.flashTimer = 15;
                smallExplosions.push(new SmallExplosion(target.x + target.width / 2, target.y + target.height / 2));
                if (target.health <= 0) handleTargetDestroyed(target, this.player);
    ...
    update() {
        ...
        // --- LÓGICA ACTUALIZADA ---
        // 1. Lógica para revelar las cadenas secuencialmente
        this.chainRevealTimer++;
        if (this.chainRevealTimer >= this.chainRevealDelay && this.visibleChainCount < this.chains.length) {
            this.visibleChainCount++;
            this.chainRevealTimer = 0;
            // Opcional: Sonido por cada salto
            // playSound(audioAssets.hit, 0.3); 
        }

        // 2. Lógica para el segundo pulso de daño con el nuevo retardo
        if (!this.hasFiredSecondPulse && this.maxLife - this.life >= this.pulseDelay) {
            this.applyDamage();
            this.hasFiredSecondPulse = true;
            playSound(audioAssets.laserShoot, 1.2);
            // Hacemos que el jugador vuelva a brillar para el segundo pulso
            if (this.player) this.player.flashTimer = 15;
        }
    }

    draw() {
        if (this.life <= 0 || this.chains.length === 0) return;

        ctx.save();
        ctx.globalCompositeOperation = 'lighter';
        
        // El alfa general del efecto para el fundido de desaparición
        const overallAlpha = Math.min(1, this.life / (this.maxLife * 0.5));

        // --- DIBUJADO SECUENCIAL ---
        // El bucle ahora solo dibuja las cadenas que son visibles
        for (let i = 0; i < this.visibleChainCount; i++) {
            const chain = this.chains[i];
            this.drawSingleChain(chain, overallAlpha);
        }
        
        ctx.restore();
    }

    // --- NUEVO: Función auxiliar para dibujar una sola cadena ---
    // Mantenemos las optimizaciones de renderizado de la versión anterior
    drawSingleChain(chain, alpha) {
        const [source, target] = chain;
        const sourceX = (source === this.player) ? source.x + source.width / 2 : source.x + source.width / 2;
        const sourceY = (source === this.player) ? source.y : source.y + source.height / 2;
        const targetX = target.x + target.width / 2;
        const targetY = target.y + target.height / 2;
        
        // 1. Dibujamos el "glow" manual que es muy performante
        // Glow exterior
        ctx.lineWidth = 22 * scaleFactor;
        ctx.strokeStyle = `rgba(173, 216, 230, ${alpha * 0.2})`;
        this.strokeSegment(sourceX, sourceY, targetX, targetY);
        ...
        // 2. Dibujamos los segmentos de PNG
        const imageAssets = [assets.chainLightningEffect_1, assets.chainLightningEffect_2, assets.chainLightningEffect_3, assets.chainLightningEffect_4].filter(img => img);
        if (imageAssets.length > 0) {
            ...
            // Usamos solo 1 o 2 segmentos por salto para mantenerlo rápido
            const segmentCount = totalDistance > 150 * scaleFactor ? 2 : 1; 
            ...
            for (let i = 0; i < segmentCount; i++) {
                ...
                const segmentStartX = sourceX + Math.cos(angle) * startOffset;
                const segmentStartY = sourceY + Math.sin(angle) * startOffset;

                ctx.save();
                ctx.globalAlpha = alpha; // Aplicamos el alfa también a los PNGs
                ctx.translate(segmentStartX, segmentStartY);
                ctx.rotate(angle);
                ctx.drawImage(randomImage, 0, -(15 * scaleFactor), segmentLength, 30 * scaleFactor);
                ctx.restore();
            }
        }
    }
    
    // Función auxiliar para no repetir código de dibujado de línea
    strokeSegment(x1, y1, x2, y2) {
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.stroke();
    ...
    class Enemy { constructor(type) { this.type = type;
         this.image = assets[`enemy${type}`];
          this.retreating = false;
           let baseWidth, baseHeight, baseSpeed, baseHealth;
            switch(type) { 
                case 1: baseWidth = 100;
                 baseHeight = 100; baseSpeed = 2.5;
                  baseHealth = 2;
                   this.shootInterval = 2200;
                    this.movementPattern = 'chaseRetreat';
                     this.behaviorState = 'advancing';
                      this.behaviorTimer = Date.now();
                    break; case 2: baseWidth = 80;
                        baseHeight = 80; baseSpeed = 4;
                         baseHealth = 1;
                        this.shootInterval = 1500;
                    break;
                case 3: baseWidth = 140;
                 baseHeight = 140;
                  baseSpeed = 1;
                   baseHealth = 5;
                    this.shootInterval = 3000;
                     break;
                case 4: baseWidth = 90; baseHeight = 90;
                       baseSpeed = 3; baseHealth = 2;
                        this.movementPattern = 'zigzag';
                         this.zigzagDir = 1;
                          this.shootInterval = 2000;
                    break; 
                case 5: baseWidth = 110;
                 baseHeight = 110;
                  baseSpeed = 1.5;
                  baseHealth = 3;
                  this.movementPattern = 'homing';
                  this.shootInterval = 1800;                  
                    break;
                case 6: baseWidth = 100;
                  baseHeight = 100;
                   baseSpeed = 2.5;
                    baseHealth = 3;
                     this.movementPattern = 'sineWave';
                      this.angle = 0;
                       this.initialX = Math.random() * (canvas.width - (baseWidth * scaleFactor));
                        this.shootInterval = 2200; break;
                case 7:
                     baseWidth = 180;
                     baseHeight = 180; 
                     baseSpeed = 0.8; baseHealth = 10; 
                     this.shootInterval = 3500; break; 
                case 8: baseWidth = 60; 
                     baseHeight = 60; 
                     baseSpeed = 2.8; 
                     baseHealth = 1; 
                     this.movementPattern = 'diveBomb'; 
                     this.behaviorState = 'patrolling'; 
                     this.speedY = baseSpeed * 0.5; 
                     this.speedX = (Math.random() < 0.5 ? 1 : -1) * baseSpeed * 0.7; 
                     break; 
                case 9: 
                    baseWidth = 70; 
                    baseHeight = 70; 
                    baseSpeed = 2.0; 
                    baseHealth = 2; 
                    this.movementPattern = 'squadron'; 
                    this.shootInterval = 2500; 
                    this.squadAngle = 0; break; 
                    case 10: baseWidth = 90; 
                    baseHeight = 90; 
                    baseSpeed = 3.5; 
                    baseHealth = 3; 
                    this.movementPattern = 'zigzagWide'; 
                    this.zigzagDir = (Math.random() < 0.5 ? 1 : -1); 
                    this.shootInterval = 1800; 
                    break; 
                    default: baseWidth = 100; 
                    baseHeight = 100; 
                    baseSpeed = 2; 
                    baseHealth = 1; 
                    this.shootInterval = 2500; } 
                    ...
                    this.x = this.initialX || Math.random() * (canvas.width - this.width); 
        ...
        if (this.flashTimer && this.flashTimer > 0) {
            ctx.save();
            ctx.globalCompositeOperation = 'lighter';
            ctx.globalAlpha = (this.flashTimer / 15) * 0.8;
            ctx.drawImage(this.image, this.x, this.y, this.width, this.height);
            ctx.restore();
            this.flashTimer--;
        }
    ...
    class Boss { constructor(bossType) { this.bossType = bossType; const bossConfigs = { 'SUPER_BOSS_1': { img: assets.superBoss1, size: 6, health: 250, attacks: ['homingBarrage', 'sweepingWall'] }, 'SUPER_BOSS_2': { img: assets.superBoss2, size: 6, health: 300, attacks: ['mineLayer', 'crossfire'] }, 'SUPER_BOSS_3': { img: assets.superBoss3, size: 6, health: 350, attacks: ['spiral', 'burstSnipe'] }, 'SUPER_BOSS_4': { img: assets.superBoss4, size: 6, health: 450, attacks: ['cone', 'summon'] }, 'GIGA_BOSS': { img: assets.gigaBoss, size: 10, health: 900, attacks: ['homingBarrage', 'sweepingWall', 'mineLayer', 'crossfire', 'spiral', 'burstSnipe', 'cone', 'summon'] }, 'FINAL_ENEMY': { img: assets.finalEnemy, size: 1.5, health: 100, attacks: ['finalBurst', 'spiral'] }, 'REGULAR': { img: assets[`enemy${bossType}`], size: 4, health: 100, attacks: ['radial', 'circular'] } }; const config = bossConfigs[bossType] || bossConfigs['REGULAR']; this.image = config.img; const playerBaseWidth = GAME_CONFIG.player.width * scaleFactor; if (this.bossType === 'GIGA_BOSS') { this.width = 1000 * scaleFactor; this.height = 500 * scaleFactor; } else { this.width = playerBaseWidth * config.size; this.height = playerBaseWidth * config.size; } this.health = config.health; this.maxHealth = this.health; this.attackPatterns = config.attacks; this.x = canvas.width / 2 - this.width / 2; this.y = 0 - this.height; this.speed = 1 * scaleFactor; this.isPositioned = false; this.isVulnerable = false; this.attackPhaseIndex = 0; this.attackPhase = this.attackPatterns[0]; this.lastAttackSwitch = Date.now(); this.lastShotTime = 0; this.circularShotIndex = 0; this.movementAngle = -Math.PI / 2; this.sweepAngle = -Math.PI / 4; this.sweepDirection = 1; if (this.bossType === 'FINAL_ENEMY') { this.speed = GAME_CONFIG.player.speed * scaleFactor * 1.5; this.moveTimer = 0; this.targetX = Math.random() * (canvas.width - this.width); this.targetY = Math.random() * (canvas.height - this.height); } } draw() { 
        ...
        if (this.image) { ctx.drawImage(this.image, this.x, this.y, this.width, this.height); } 
        if (this.isPositioned && this.bossType !== 'FINAL_ENEMY') { 
            const barWidth = canvas.width / 2; const barHeight = 25 * scaleFactor; 
            const barY = 30 * scaleFactor; const healthPercentage = this.health / this.maxHealth; 
            ctx.fillStyle = '#440000'; 
            ctx.fillRect(canvas.width / 2 - barWidth / 2, barY, barWidth, barHeight); 
            ctx.fillStyle = '#00ff00'; 
            ctx.fillRect(canvas.width / 2 - barWidth / 2, barY, barWidth * healthPercentage, barHeight); 
            ctx.strokeStyle = 'white'; 
            ctx.strokeRect(canvas.width / 2 - barWidth / 2, barY, barWidth, barHeight); } } 
        update() { 
            if (players.every(p => p.isDefeated)) {
            return;
        }
        if (this.bossType === 'FINAL_ENEMY') { 
            if (!this.isPositioned) { 
                ...
                if (this.moveTimer <= 0) { 
                    if (Math.random() < 0.3) { 
                        this.targetX = players[0].x > canvas.width / 2 ? Math.random() * (canvas.width / 4) : canvas.width * 0.75 + Math.random() * (canvas.width / 4); 
                        this.targetY = players[0].y > canvas.height / 2 ? Math.random() * (canvas.height / 4) : canvas.height * 0.75 + Math.random() * (canvas.height / 4); 
                    } else { 
                        this.targetX = Math.random() * (canvas.width - this.width); this.targetY = Math.random() * (canvas.height * 0.8); } 
                        this.moveTimer = Math.random() * 1000 + 500; } 
                        const angle = Math.atan2(this.targetY - this.y, this.targetX - this.x); 
                        this.x += Math.cos(angle) * this.speed; this.y += Math.sin(angle) * this.speed; 
                    } else { 
                        if (!this.isPositioned) { 
                            if (this.y < (50 * scaleFactor)) { 
                            this.y += this.speed; 
                        } else { 
                            this.isPositioned = true; this.isVulnerable = true; 
                        } return; } 
                        this.movementAngle += 0.01; 
                        const moveRange = (this.bossType === 'GIGA_BOSS') ? (200 * scaleFactor) : (100 * scaleFactor); 
                        this.x = (canvas.width / 2 - this.width / 2) + Math.cos(this.movementAngle) * moveRange; 
                        if (this.bossType === 'GIGA_BOSS') { this.y = (50 * scaleFactor) + Math.sin(this.movementAngle * 0.7) * (40 * scaleFactor); 

                        } } 
                        if (Date.now() - this.lastAttackSwitch > 10000) { this.attackPhaseIndex = (this.attackPhaseIndex + 1) % this.attackPatterns.length; 
                            this.attackPhase = this.attackPatterns[this.attackPhaseIndex]; 
                            this.lastAttackSwitch = Date.now(); 
                            this.circularShotIndex = 0; } 
                            this.shoot(); } 
                            shoot() { 
                                switch(this.attackPhase) { case 'radial': if (Date.now() - this.lastShotTime > 4000) { 
                                    this.lastShotTime = Date.now(); 
                                    setTimeout(() => this.fireRadialBurst(), 0); 
                                    setTimeout(() => this.fireRadialBurst(), 500); } 
                                    break; 
                                case 'circular': if (Date.now() - this.lastShotTime > 150) { 
                                    this.lastShotTime = Date.now(); 
                                    this.fireCircularShot(); } 
                                    break; 
                                case 'homingBarrage': if (Date.now() - this.lastShotTime > 2000) { this.lastShotTime = Date.now(); 
                                    this.firePredictiveBurst(); } 
                                    break; 
                                case 'sweepingWall': if (Date.now() - this.lastShotTime > 50) { this.lastShotTime = Date.now(); 
                                    this.fireSweepingWall(); } 
                                    break; 
                                    case 'mineLayer': if (Date.now() - this.lastShotTime > 1000) { this.lastShotTime = Date.now(); 
                                        this.fireMine(); } 
                                        break; 
                                    case 'crossfire': if (Date.now() - this.lastShotTime > 200) { this.lastShotTime = Date.now(); 
                                        this.fireCrossfire(); } 
                                        break; 
                                    case 'spiral': if (Date.now() - this.lastShotTime > 60) { this.lastShotTime = Date.now(); 
                                        this.fireSpiral(); } 
                                        break; 
                                    case 'burstSnipe': if (Date.now() - this.lastShotTime > 2000) { this.lastShotTime = Date.now(); 
                                        this.fireBurstSnipe(); } 
                                        break; 
                                    case 'cone': if (Date.now() - this.lastShotTime > 1500) { this.lastShotTime = Date.now(); 
                                        this.fireCone(); } 
                                        break; 
                                    case 'summon': 
                                        if (Date.now() - this.lastShotTime > 8000) { this.lastShotTime = Date.now(); 
                                            this.summonEnemies(); } 
                                            break; 
                                    case 'finalBurst': 
                                        if (Date.now() - this.lastShotTime > 1500) { this.lastShotTime = Date.now(); 
                                            this.fireFinalBurst(); } 
                                            break; 
                                        } } 
                                        fireRadialBurst() { playSound(audioAssets.enemyShoot, 0.6); 
                                            const bulletSpeed = 5; const wingLeftX = this.x + this.width * 0.1; 
                                            const wingRightX = this.x + this.width * 0.9; 
                                            const wingY = this.y + this.height * 0.7; 
                                            for (let i = 0; i < 8; i++) { 
                                                const angle = (i / 8) * Math.PI * 2; 
                                                const speedX = Math.cos(angle) * bulletSpeed; 
                                                const speedY = Math.sin(angle) * bulletSpeed; 
                                                enemyBullets.push(new EnemyBullet(wingLeftX, wingY, speedX, speedY)); 
                                                enemyBullets.push(new EnemyBullet(wingRightX, wingY, speedX, speedY)); } } 
                                        fireCircularShot() { playSound(audioAssets.enemyShoot, 0.3); 
                                            const bulletSpeed = 6; 
                                            const baseAngle = Math.atan2(0, -Math.sin(this.movementAngle)); 
                                            const angle = baseAngle + (this.circularShotIndex / 15) * Math.PI * 2; const speedX = Math.cos(angle) * bulletSpeed; 
                                            const speedY = Math.sin(angle) * bulletSpeed; 
                                            enemyBullets.push(new EnemyBullet(this.x + this.width / 2, this.y + this.height * 0.8, speedX, speedY)); 
                                            this.circularShotIndex++; 
                                            if (this.circularShotIndex >= 15) { this.circularShotIndex = 0; 
                                                this.lastShotTime = Date.now() + 2000; } } 
                                        firePredictiveBurst() { playSound(audioAssets.enemyShoot, 0.7); 
                                            const bulletSpeed = 8; const centerX = this.x + this.width / 2; 
                                            const centerY = this.y + this.height * 0.8; 
                                            const angleToPlayer = Math.atan2(players[0].y - centerY, players[0].x - centerX); 
                                            for(let i = -1; i <= 1; i++){ const angle = angleToPlayer + i * 0.2; 
                                                enemyBullets.push(new EnemyBullet(centerX, centerY, Math.cos(angle) * bulletSpeed, Math.sin(angle) * bulletSpeed)); } } 
                                        fireSweepingWall() { playSound(audioAssets.enemyShoot, 0.2); 
                                            const bulletSpeed = 7; const baseAngle = Math.PI / 2; 
                                            const angle = baseAngle + this.sweepAngle; 
                                            const speedX = Math.cos(angle) * bulletSpeed; 
                                            const speedY = Math.sin(angle) * bulletSpeed; 
                                            enemyBullets.push(new EnemyBullet(this.x + this.width / 2, this.y + this.height * 0.8, speedX, speedY)); 
                                            this.sweepAngle += 0.05 * this.sweepDirection; if (this.sweepAngle > Math.PI / 4 || this.sweepAngle < -Math.PI / 4) { this.sweepDirection *= -1; } } 
                                        fireMine() { playSound(audioAssets.enemyShoot, 0.5); 
                                            const mineX = this.x + Math.random() * this.width; 
                                            enemyBullets.push(new MineBullet(mineX, this.y + this.height * 0.7));
...
function resetPlayerStats(player, fullReset = false) {
    ...
    clearTimeout(player.heavyCannonTimeoutId);
    clearTimeout(player.laserTimeoutId);
    clearTimeout(player.slowShotTimeoutId);
    clearTimeout(player.rapidFireTimeoutId);
    clearInterval(player.missileChargeIntervalId);
    player.missileChargeIntervalId = null;
}
...
function initGame(startProgressionIndex = 0) {
        ...
        if (applyAllPowerupsCheat) {
            players.forEach(p => {
                ...
                p.shieldStacks = 3;
                clearInterval(p.missileChargeIntervalId); 
        
            // Creamos un nuevo intervalo específico para ESTE jugador (p)
            p.missileChargeIntervalId = setInterval(() => {
             if (gameRunning && !isPaused && p.missileCharges < GAME_CONFIG.missiles.maxCharges) {
                 p.missileCharges++;
             }
             }, GAME_CONFIG.missiles.chargeTime); // Démosle escudos también
            });
...
function createMobilePauseButton() {
    ...
    function spawnWave(count) { for (let i = 0; i < count; i++) {
...
gameRunning = false; isPaused = false; playMusic(null); [difficultyTimer, asteroidInterval, meteorShowerTimer, aggressiveAsteroidSpawner, waveTimer].forEach(timer => { if(timer) {clearInterval(timer); clearTimeout(timer);} }); const pauseMenu = document.getElementById('pauseMenu'); if (pauseMenu) gameContainer.removeChild(pauseMenu); canvas.style.display = 'none'; lobby.style.display = 'flex'; updateLobbyUI(); }
function damagePlayer(player, amount = 1) { 
    if (player.isInvulnerable) return; 

    // Primero, verificamos si el AURA está activa (que corresponde a 4 escudos).
    if (player.shieldStacks === 4) {
        player.shieldStacks = 3; // El daño consume el aura, dejándote con 3 escudos.
        player.isAuraActive = false;
        
        // El aura ya no te hace invulnerable, pero te damos el flash de invulnerabilidad normal.
        player.isInvulnerable = true;
        setTimeout(() => { player.isInvulnerable = false; }, 400);

        // Hacemos que el visual del aura se desvanezca.
        if (player.auraVisual) {
            player.auraVisual.state = 'disappearing';
        }

        // --- INICIO DE LA CORRECCIÓN #1 (AURA ROTA) ---
        // Al romperse el aura, necesitamos reconstruir los 3 escudos visuales que quedan.
        // Esto faltaba en tu código original.
        player.shieldVisuals = []; // Limpiamos por si acaso.
        for (let i = 1; i <= 3; i++) {
            const shieldImage = assets[`shieldEffect${i}`];
            if (shieldImage) {
        ...
        return; // Salimos de la función, el daño fue absorbido por el aura.
    }

    // Si no hay aura, manejamos los escudos normales (1 a 3).
    if (player.shieldStacks > 0) {
        player.shieldStacks--;
        
        // --- INICIO DE LA CORRECCIÓN #2 (ERROR ORIGINAL) ---
        // Este es el cambio clave que te mencioné en la respuesta anterior.
        // Eliminamos el .pop() y en su lugar solo marcamos el escudo para que desaparezca.
        if (player.shieldVisuals.length > 0) {
            // Obtenemos el último escudo visual SIN removerlo del array
            const consumedShield = player.shieldVisuals[player.shieldVisuals.length - 1];
            // Le decimos que inicie su animación de desvanecimiento.
            consumedShield.state = 'disappearing'; 
        }
        ...
        player.isInvulnerable = true;
        setTimeout(() => { player.isInvulnerable = false; }, 400);
        return; // Salimos, el daño fue absorbido.
    }
    ...
    for (let i = 0; i < bullets.length; ) {
        ...
        for (const target of allTargets) {
            ...
            if (checkCircularCollision(bullet, target)) {
                ...
                if (target.health <= 0) {
                    handleTargetDestroyed(target, bullet.owner); 
                }
    ...
    for (let i = 0; i < missiles.length; ) {
        ...
        for (const target of allTargets) {
            ...
            if (checkCircularCollision(missile, target)) {
                ...
                if (target.health <= 0) {
                    handleTargetDestroyed(target, missile.owner); 
                }
    ...
    for (let i = 0; i < largeAsteroids.length; i++) {
        for (let j = i + 1; j < largeAsteroids.length; j++) {
            ...
            if (Math.abs(dx) < combinedHalfWidths && Math.abs(dy) < combinedHalfHeights) {
                ...
                const pushFactor = 0.25; // Factor de empuje suave
...
laser.collidedEnemies.add(target);
 ...
 if (target.health <= 0) { handleTargetDestroyed(target, laser.owner); } } }
            });
        });
    ...
    function handleTargetDestroyed(target, scoringPlayer) {
    if (!scoringPlayer || scoringPlayer.isDefeated) {
        const fallbackPlayer = players.find(p => !p.isDefeated);
        if (!fallbackPlayer) return; 
        scoringPlayer = fallbackPlayer;
    }
    if (target instanceof Boss) {
        ...
        if (isTwoPlayerMode) {
            if (players[0] && !players[0].isDefeated) {
                 homingPowerUps.push(new HomingPowerUp(target.x + target.width / 2, target.y + target.height / 2, 'level', players[0]));
            }
            if (players[1] && !players[1].isDefeated) {
                 homingPowerUps.push(new HomingPowerUp(target.x + target.width * 0.7, target.y + target.height / 2, 'level', players[1]));
            }
        } else if (players[0]) { // Mantiene el comportamiento para 1 jugador
             homingPowerUps.push(new HomingPowerUp(target.x + target.width / 2, target.y + target.height / 2, 'level', players[0]));
        }
        ...
        switch (target.type) {
            case 4: //... Asteroide Gigante -> genera 1 Mediano (2) y 1 Común (1)
                queueFragment(2, target.x, target.y); 
                queueFragment(1, target.x + target.width * 0.5, target.y + target.height * 0.5);
                break;
            case 2: //... Asteroide Mediano -> genera 1 Común (1) y 1 Rápido (3)
                queueFragment(1, target.x, target.y);
                queueFragment(3, target.x + target.width * 0.5, target.y + target.height * 0.5);
                break;
            case 1: //... Asteroide Común -> genera 1 Rápido (3)
                queueFragment(3, target.x + target.width * 0.25, target.y + target.height * 0.25);
                break;
            case 5: //... Asteroide Rápido (Variante) -> Suelta un imán (sin cambios)
                powerUps.push(new PowerUp(target.x, target.y, 'magnet'));
                break;
            case 3: //... Asteroide Rápido -> Probabilidad de soltar power-up (sin cambios)
                if (Math.random() < powerUpSpawnChance) {
                    ...
                    if (Math.random() < 0.10) type = 'magnet';
        ...
        } else if (Math.random() < (isLuckActive ? 0.9 : powerUpSpawnChance)) {
            const rand = Math.random(); let type = 'health';
            if (Math.random() < 0.05) {
                type = 'luck';
            } else {
    ...
    // 1. Configura los fondos ACTUALES basados en el índice de inicio
    currentBgFar = assets.backgroundsFar[currentBackgroundIndex];
    currentBgMid = assets.backgroundsMid[currentBackgroundIndex];
    currentBgNear = assets.backgroundsNear[currentBackgroundIndex];
    ...
    // 2. Prepara los fondos SIGUIENTES una sola vez al inicio
    const nextIndex = (currentBackgroundIndex + 1) % assets.backgroundsFar.length;
    nextBgFar = assets.backgroundsFar[nextIndex];
    nextBgMid = assets.backgroundsMid[nextIndex];
    nextBgNear = assets.backgroundsNear[nextIndex];
    ...
    // 3. Ya no necesitamos la variable de estado global
...
function applyPowerUp(player, type) {  
    ...
    switch (type) {
         case 'luck':
            playSound(audioAssets.powerupExtraLife, 1.2); 
            clearTimeout(luckTimeout); // Luck sigue siendo global, puede ser un beneficio para el equipo
            isLuckActive = true;
        ...
        case 'shield':
            ...
            player.shieldStacks++;
            player.shieldVisuals.forEach(sv => sv.state = 'disappearing'); // Les decimos a los viejos que se desvanezcan
            player.shieldVisuals = []; 
            if (player.shieldStacks === 4) {
                ...
                // El aura dura 8 segundos o hasta que te golpeen.
                // Si no te golpean, al terminar, te deja con 3 escudos.
                setTimeout(() => {
        // Comprobamos si el aura sigue activa después de 7 segundos
        if (player.shieldStacks === 4) { 
            triggerScreenFlash(200, 0.6); 
            playSound(audioAssets.powerdown, 0.8);

            if (player.auraVisual && assets.shieldEffect5) {
                player.auraVisual.image = assets.shieldEffect5;
            }

            // FASE 2: Dura 1 segundo más con el visual de advertencia.
            setTimeout(() => {
                // Volvemos a comprobar por si nos golpearon en el último segundo
                if (player.shieldStacks === 4) { 
                    player.shieldStacks = 3;
                    player.isAuraActive = false;
                    player.isInvulnerable = false;
                    if (player.auraVisual) {
                        player.auraVisual.state = 'disappearing';
                    ...
                    // --- INICIO DE LA CORRECCIÓN #3 (AURA EXPIRADA) ---
                    // Limpiamos el array de visuales ANTES de reconstruir los 3 escudos.
                    // Esto evita que se acumulen escudos invisibles.
                    player.shieldVisuals = []; 
                    for (let i = 1; i <= 3; i++) {
                         const shieldImage = assets[`shieldEffect${i}`];
                         if (shieldImage) player.shieldVisuals.push(new ShieldVisual(player, shieldImage));
                    }
            ...
            // Reconstruimos los escudos visuales normales (1 a 3) según el contador
            // Esto se ejecuta tanto si recoges un escudo normal como si el aura se activa
            for (let i = 1; i <= Math.min(3, player.shieldStacks); i++) {
                const shieldImage = assets[`shieldEffect${i}`];
                if (shieldImage) {
                    player.shieldVisuals.push(new ShieldVisual(player, shieldImage));
                }
            ...
            break;
        case 'magnet':
            playSound(audioAssets.powerupDrone, 0.8);
        ...
        case 'rapidFire':
            playSound(audioAssets.powerupBurst);
            clearTimeout(player.rapidFireTimeoutId); // Limpia el temporizador del jugador
            player.burstFireLevel = Math.min(3, player.burstFireLevel + 1); 
            if (player.burstFireLevel === 3) { 
                player.rapidFireTimeoutId = setTimeout(() => { // Asigna al jugador
       ...
       case 'heavyCannon': { 
            ...
            // --- Lógica del Disparador del Relámpago (se mueve arriba) ---
            const hadLaserBefore = player.laserLevel > 0; 
            const hasWings = player.wingCannonsLevel > 0;
            const hasBurst = player.burstFireLevel > 0;

            if (hasWings && hasBurst && hadLaserBefore) {
               const nearestTarget = findNearestTarget(player.x, player.y, ['enemies', 'bosses']);
                const searchRadius = GAME_CONFIG.chainLightning.searchRadius * scaleFactor;
                
                if (nearestTarget) {
                    const dx = nearestTarget.x - player.x;
                    const dy = nearestTarget.y - player.y;
                    // Si el enemigo más cercano está dentro del rango, disparamos
                    if (dx * dx + dy * dy < searchRadius * searchRadius) {
                        chainLightnings.push(new ChainLightning(player));
                    } else {
                        // Si no, guardamos la carga
                        player.hasChainLightningCharge = true;
                        player.flashTimer = 15; // Feedback visual de que la carga está lista
                        playSound(audioAssets.powerupDrone, 0.8);
                    }
                } else {
                    // Si no hay enemigos en pantalla, guardamos la carga
                    player.hasChainLightningCharge = true;
                    player.flashTimer = 15;
                    playSound(audioAssets.powerupDrone, 0.8);
                }
            }
            // --- Fin de la lógica del disparador ---

            player.heavyCannonLevel = Math.min(5, player.heavyCannonLevel + 1);
            player.laserLevel = 0;
            clearTimeout(player.heavyCannonTimeoutId);
            clearTimeout(player.laserTimeoutId);
            player.heavyCannonTimeoutId = setTimeout(() => {
                player.heavyCannonLevel = 0;
            }, 15000);
            break;
        }
        case 'laser': { // <-- Llave de apertura
            playSound(audioAssets.powerupHeavy);

            // --- Lógica del Disparador del Relámpago (se mueve arriba) ---
            const hadHeavyBefore = player.heavyCannonLevel > 0;
            const hasWings = player.wingCannonsLevel > 0;
            const hasBurst = player.burstFireLevel > 0;

            if (hasWings && hasBurst && hadHeavyBefore) {
               const nearestTarget = findNearestTarget(player.x, player.y, ['enemies', 'bosses']);
                const searchRadius = GAME_CONFIG.chainLightning.searchRadius * scaleFactor;
            ...
            clearTimeout(player.heavyCannonTimeoutId);
            clearTimeout(player.laserTimeoutId);
            player.laserTimeoutId = setTimeout(() => {
                player.laserLevel = 0;
        ...
        case 'missileSystem':
            ...
            if (!player.missileSystemActive) { 
                ...
                // Limpia cualquier intervalo anterior de este jugador por seguridad
                clearInterval(player.missileChargeIntervalId);
                // Asigna el nuevo intervalo a la propiedad del jugador
                player.missileChargeIntervalId = setInterval(() => {
        ...
        case 'slowShot':
            ...
            clearTimeout(player.slowShotTimeoutId); // Limpia el temporizador del jugador
            player.slowShotTimeoutId = setTimeout(() => { // Asigna al jugador
                player.slowShotStacks = 0; 
            }, GAME_CONFIG.powerups.slowShotDuration);
            break;
        case 'level':
            playSound(audioAssets.powerupExtraLife, 1.0);
            player.levelPowerUpStacks++;
            player.levelPowerShotCooldown = Math.max(500, 10000 - (player.levelPowerUpStacks * 1000));
   ...
   function gameLoop() {
    ...
    if (players.length > 0 && players.every(p => p.isDefeated) && gameState !== 'GAME_OVER_DELAY') {
    ...
    // 3. Dibujamos las siguientes capas (si existen) para una transición suave
    ...
    // 4. LÓGICA DE TRANSICIÓN INDEPENDIENTE PARA CADA CAPA
    
    // --- Capa LEJANA (Far) ---
    if (bgFarY >= renderedFarHeight) {
        bgFarY -= renderedFarHeight;
        preRenderedBgFar = nextPreRenderedBgFar; // El 'siguiente' se convierte en el 'actual'
        
        // Preparamos inmediatamente el próximo fondo para esta capa
        const nextIndex = (assets.backgroundsFar.indexOf(nextBgFar) + 1) % assets.backgroundsFar.length;
        nextBgFar = assets.backgroundsFar[nextIndex];
        nextPreRenderedBgFar = preRenderBackground(nextBgFar);
    }

    // --- Capa MEDIA (Mid) ---
    if (bgMidY >= renderedMidHeight) {
        bgMidY -= renderedMidHeight;
        preRenderedBgMid = nextPreRenderedBgMid;

        const nextIndex = (assets.backgroundsMid.indexOf(nextBgMid) + 1) % assets.backgroundsMid.length;
        nextBgMid = assets.backgroundsMid[nextIndex];
        nextPreRenderedBgMid = preRenderBackground(nextBgMid);
    }
    
    // --- Capa CERCANA (Near) ---
    if (bgNearY >= renderedNearHeight) {
        bgNearY -= renderedNearHeight;
        preRenderedBgNear = nextPreRenderedBgNear;
        
        const nextIndex = (assets.backgroundsNear.indexOf(nextBgNear) + 1) % assets.backgroundsNear.length;
        nextBgNear = assets.backgroundsNear[nextIndex];
        nextPreRenderedBgNear = preRenderBackground(nextBgNear);
    }
    ...
    // --- 2. LÓGICA DE ESTADO DEL JUEGO ---
        if (gameState === 'GAME_OVER_DELAY') {
        // Comprobamos si han pasado 3 segundos (3000 ms)
        if (Date.now() - gameOverDelayStartTime > 3000) {
            // Si pasaron, ejecutamos la lógica original de Game Over
            gameRunning = false;
            playMusic(null);
            [difficultyTimer, asteroidInterval, meteorShowerTimer, aggressiveAsteroidSpawner, waveTimer].forEach(timer => {
                if (timer) { clearInterval(timer); clearTimeout(timer); }
            });
            // Recopilamos las estadísticas finales de TODOS los jugadores
            const finalPlayerStats = players.map(p => ({
                player: p, // Guardamos la referencia al jugador
                score: p.score,
                kills: p.enemyDestroyedCount,
                level: p.bossesDestroyed + 1,
                difficulty: difficultyLevel + 1 // Sumamos 1 para que sea de 1 a 6
            }));
            showGameOverScreen(finalPlayerStats);
            return; // Detenemos la ejecución de este frame
        }
    }
    ...
    if (gameRunning && !isPaused) {
        if (isTwoPlayerMode) {
            ...
            if (keys['g'] && players[0] && !players[0].isDefeated) players[0].shoot();
            if (keys['1'] && players[1] && !players[1].isDefeated) players[1].shoot();
        } else {
            if (keys[' '] && players[0] && !players[0].isDefeated) players[0].shoot();
        }
    }
    stars.forEach(s => s.update(playerSpeedX, playerSpeedY));
    players.forEach(p =>{if (!p.isDefeated) p.update();});
    // Comprobar si algún jugador debe disparar un relámpago cargado
    players.forEach(player => {
        if (player.hasChainLightningCharge && !player.isDefeated) {
            const nearestTarget = findNearestTarget(player.x, player.y, ['enemies', 'bosses']);
            const searchRadius = GAME_CONFIG.chainLightning.searchRadius * scaleFactor;
            ...
            if (nearestTarget) {
                ...
                if (dx * dx + dy * dy < searchRadius * searchRadius) {
                    chainLightnings.push(new ChainLightning(player));
                    player.hasChainLightningCharge = false; // Consumir la carga
                }
            }
        }
    });
    ...
    chainLightnings.forEach((cl, i) => {
    cl.update();
    if (cl.life <= 0) chainLightnings.splice(i, 1);
});
    ...
    chainLightnings.forEach(cl => cl.draw());
...
if (gameState === 'INTERMISSION' && intermissionData) {
        ...
        if (player.isAuraActive) {
            ...
            hazards.forEach(hazard => {
                if (checkCircularCollision(player, hazard)) {
                    ...
                    if (hazard.health <= 0) {
                        handleTargetDestroyed(hazard, player);
                    }
    ...
    for (let i = 0; i < enemiesToSpawn; i++) {
        const randomType = Math.floor(Math.random() * 10) + 1;
        setTimeout(() => { if(gameRunning && enemies.length < GAME_CONFIG.gameplay.maxEnemiesOnScreen) {
                enemies.push(new Enemy(randomType)); 
    ...
    function showGameOverScreen(finalPlayerStats) { 
    const mobilePauseBtn = document.getElementById('mobilePauseButton');
    if (mobilePauseBtn) {
        mobilePauseBtn.remove();
    }

    // Filtramos los jugadores cuyo puntaje califica para el ranking
    const highScorePlayers = finalPlayerStats.filter(stats => checkIfHighScore(stats.score));

    if (highScorePlayers.length > 0) {
        // Si al menos uno califica, mostramos la pantalla de ingreso de iniciales
        showHighScoreInputScreen(finalPlayerStats, false); // false porque no es una victoria
    } else {
        // --- INICIO DE LA MODIFICACIÓN ESTÉTICA ---
        // Si nadie califica, mostramos la pantalla de Game Over tradicional con el nuevo estilo
        ...
        let statsHTML = '';
        finalPlayerStats.forEach((stats, index) => {
            statsHTML += `<p style="font-size: 1.5em; margin: 10px 0;">P${index + 1} Score: ${stats.score}</p>`;
        });

        gameOverDiv.innerHTML = `
            <h1 style="color: #ff4444; font-size: 3.5em; text-shadow: 2px 2px 8px #f00; margin-top: 0;">GAME OVER</h1>
            ${statsHTML}
            <button id="restartButton" style="font-family: 'VT323', monospace; margin-top: 25px; padding: 12px 25px; font-size: 1.5em; background-color: #333; color: #fff; border: 2px solid #fff; cursor: pointer;">Volver al Menú</button>
        `;
...
function showVictoryScreen() { 
    ...
    const finalPlayerStats = players.map(p => ({
        ...
        score: p.score,
        kills: p.enemyDestroyedCount,
        level: "00", // Nivel especial para victoria
        difficulty: difficultyLevel + 1
    }));
    
    // Para la victoria, siempre mostramos la pantalla de ingreso de iniciales
    // ya que es un logro mayor.
    showHighScoreInputScreen(finalPlayerStats, true); // true porque es una victoria
}
/**
 * Muestra la pantalla para que los jugadores ingresen sus iniciales.
 * @param {Array} finalPlayerStats - Estadísticas de todos los jugadores al final de la partida.
 * @param {boolean} isVictory - True si es la pantalla de victoria.
 */
function showHighScoreInputScreen(finalPlayerStats, isVictory) {
    isHighScoreInputActive = true;
    highScoreEntryData = finalPlayerStats.filter(stats => checkIfHighScore(stats.score));
    
    // Inicializamos las iniciales para cada jugador que calificó
    currentInitials = highScoreEntryData.map(() => ['A', 'A', 'A']);
    activePlayerInputIndex = 0;
    activeLetterInputIndex = 0;

    const screenDiv = document.createElement('div');
    screenDiv.id = 'highScoreScreen';
    
    const titleColor = isVictory ? '#00ff00' : '#ff4444';
    const borderColor = isVictory ? '#00ff00' : '#ff4444';
    const titleText = isVictory ? '¡VICTORIA!' : '¡NUEVO RÉCORD!';
    const titleShadow = isVictory ? '2px 2px 8px #0f0' : '2px 2px 8px #f00';
    ...
    // --- FIN DE LAS MODIFICACIONES ESTÉTICAS ---
    ...
    let contentHTML = `
        <style>
            ...
            .letterBox.active {
                ...
                box-shadow: 0 0 10px ${titleColor};
            }
            #saveScoresButton {
    ...
    finalPlayerStats.forEach((stats) => {
        const isHighScore = highScoreEntryData.includes(stats);
        const playerIndexInHighScore = highScoreEntryData.indexOf(stats);

        contentHTML += `<div class="playerRow">`;
        contentHTML += `<span>P${stats.player.id}</span>`;
        contentHTML += `<span>Score: ${stats.score}</span>`;
        contentHTML += `<span>Nivel: ${stats.level}</span>`;
        contentHTML += `<span>Kills: ${stats.kills}</span>`;

        if (isHighScore) {
            contentHTML += `<div class="initialsContainer">
                <span class="letterBox" id="initial-${playerIndexInHighScore}-0">A</span>
                <span class="letterBox" id="initial-${playerIndexInHighScore}-1">A</span>
                <span class="letterBox" id="initial-${playerIndexInHighScore}-2">A</span>
            </div>`;
        } else {
             contentHTML += `<div></div>`; // Placeholder for alignment
        }
        contentHTML += `</div>`;
    });

    contentHTML += `<button id="saveScoresButton">Guardar y Salir</button>`;
    screenDiv.innerHTML = contentHTML;
    gameContainer.appendChild(screenDiv);

    document.getElementById('saveScoresButton').onclick = saveEntriesAndExit;
    updateInputUI(); // Resalta la primera letra
}

/**
 * Resalta la letra activa en la interfaz.
 */
function updateInputUI() {
    document.querySelectorAll('.letterBox').forEach(box => box.classList.remove('active'));
    const activeBox = document.getElementById(`initial-${activePlayerInputIndex}-${activeLetterInputIndex}`);
    if (activeBox) {
...
/**
 * Guarda las entradas del ranking y vuelve al lobby.
 */
function saveEntriesAndExit() {
    highScoreEntryData.forEach((stats, index) => {
        const newScoreEntry = {
            name: currentInitials[index].join(''),
            score: stats.score,
            level: stats.level,
        ...
        addHighScore(newScoreEntry);
    });

    isHighScoreInputActive = false;
    const screenDiv = document.getElementById('highScoreScreen');
    if (screenDiv) gameContainer.removeChild(screenDiv);
    exitToLobby();
}    
async function startGame(startProgressionIndex = 0) {
    ...
    /**
 * Formatea una entrada de puntuación para mostrarla en el lobby.
 * @param {object} entry - El objeto de la puntuación.
 * @param {number} rank - La posición en el ranking (1, 2, 3...).
 * @returns {string} - La línea de texto formateada.
 */
function formatScoreEntry(entry, rank) {
    const rankStr = String(rank).padStart(2, ' ');
    const nameStr = entry.name.toUpperCase().padEnd(5, ' ');
    const levelStr = String(entry.level).padStart(3, ' ');
    const diffStr = String(entry.difficulty).padStart(3, ' ');
    const killsStr = String(entry.kills).padStart(7, ' ');
    const scoreStr = String(entry.score).padStart(12, ' ');

    return `${rankStr}. ${nameStr} ${levelStr}   ${diffStr}   ${killsStr}   ${scoreStr}`;
}
    function updateLobbyUI() {
        ...
        lobby.innerHTML = `
            <div id="mainMenu" style="flex: 1; padding: 20px;">
                ...
                <button id="twoPlayerButton" style="font-family: 'VT323', monospace; margin-top: 20px; padding: 12px 25px; font-size: 1.5em; background-color: ${isTwoPlayerMode ? '#00ff00' : '#555'}; color: ${isTwoPlayerMode ? '#000' : '#fff'}; border: 1px solid #fff;">x2 Player ${isTwoPlayerMode ? 'Activado' : 'Desactivado'}</button></div>
            ...
            <div id="highScoreDisplay" style="flex: 1; padding: 20px;">
                </div>
        ...
        const highScoreContainer = lobby.querySelector('#highScoreDisplay');
    const highScores = getHighScores();
    let highScoreHTML = '<h2 style="text-align: center; color: #00ff00;">MEJORES PUNTAJES</h2>';
    highScoreHTML += '<pre style="font-family: \'VT323\', monospace; font-size: 1.1em; line-height: 1.6;">';
    highScoreHTML += 'N°. NOMBRE LVL   DIF     KILLS          SCORE\n';
    highScoreHTML += '-----------------------------------------------------\n';

    if (highScores.length > 0) {
        highScores.forEach((score, index) => {
            highScoreHTML += `${formatScoreEntry(score, index + 1)}\n`;
        });
    } else {
        highScoreHTML += 'Aún no hay récords. ¡Sé el primero!\n';
    }
...
function skipIntroToEnd() {
  ...
  updateLobbyUI();
     ...
     window.addEventListener('keydown', (e) => {
        // Primero, manejamos la interfaz de ranking si está activa.
        if (isHighScoreInputActive) {
            e.preventDefault(); // Evita que las flechas muevan la página
            const key = e.key.toLowerCase();
            
            // Si no hay jugadores para ingresar iniciales, no hacemos nada.
            if (!highScoreEntryData[activePlayerInputIndex]) return;

            const currentLetter = currentInitials[activePlayerInputIndex][activeLetterInputIndex];
            ...
            switch (key) {
                ...
                case 'arrowdown':
                    ...
                    break;
                case 'arrowleft':
                    activeLetterInputIndex = Math.max(0, activeLetterInputIndex - 1);
                    break;
                case 'arrowright':
                    activeLetterInputIndex = Math.min(2, activeLetterInputIndex + 1);
                    break;
                case 'enter':
                    activeLetterInputIndex++;
                    if (activeLetterInputIndex > 2) {
                        activeLetterInputIndex = 0;
                        activePlayerInputIndex++;
                        if (activePlayerInputIndex >= highScoreEntryData.length) {
                            saveEntriesAndExit();
            ...
            // Actualizamos la letra solo si es una flecha arriba/abajo
            if(key === 'arrowup' || key === 'arrowdown') {
                currentInitials[activePlayerInputIndex][activeLetterInputIndex] = String.fromCharCode(charCode);
                const activeBox = document.getElementById(`initial-${activePlayerInputIndex}-${activeLetterInputIndex}`);
                if(activeBox) activeBox.textContent = currentInitials[activePlayerInputIndex][activeLetterInputIndex];
            }

            updateInputUI();
            return; // Detenemos la ejecución aquí para que no afecte al juego.
        }

        // --- Si la interfaz de ranking NO está activa, se ejecuta la lógica del juego ---
        const key = e.key.toLowerCase();
        keys[key] = true;
        ...
        if (gameRunning && !isPaused) {
            ...
            if (isTwoPlayerMode) {
                if (players[0] && !players[0].isDefeated && key === 'h') launchMissile(players[0]);
                if (players[1] && !players[1].isDefeated && key === '2') launchMissile(players[1]);
            } 
            // Misil para 1 jugador
            else {
                if (players[0] && !players[0].isDefeated && key === 'control') {
                    e.preventDefault();
...
function launchMissile(player) { // <--- AHORA RECIBE EL JUGADOR
    if (player && gameRunning && !isPaused && player.missileCharges > 0) {
        ...
        if (target) {
            missiles.push(new Missile(player.x + player.width / 2 - (10 * scaleFactor), player.y, target, player));
...
// =========================================================================
// --- INICIO: LÓGICA DE HIGH SCORES ---
// =========================================================================

const MAX_HIGH_SCORES = 20;

/**
 * Obtiene la lista de puntuaciones desde localStorage.
 * Devuelve un array de objetos de puntuación, o un array vacío si no hay nada.
 */
function getHighScores() {
    try {
        const scoresJSON = localStorage.getItem('spaceShooterHighScores');
        return scoresJSON ? JSON.parse(scoresJSON) : [];
    } catch (e) {
        console.error("Error al leer los high scores:", e);
        return [];
    }
}

/**
 * Guarda una lista de puntuaciones en localStorage.
 * @param {Array} scores - El array de objetos de puntuación a guardar.
 */
function saveHighScores(scores) {
    try {
        localStorage.setItem('spaceShooterHighScores', JSON.stringify(scores));
    } catch (e) {
        console.error("Error al guardar los high scores:", e);
    }
}

/**
 * Comprueba si un puntaje es lo suficientemente alto para entrar en el ranking.
 * @param {number} score - El puntaje a comprobar.
 * @returns {boolean} - True si el puntaje entra en la lista, false si no.
 */
function checkIfHighScore(score) {
    ...
    // Compara con el puntaje más bajo de la lista (que está al final)
    const lowestScoreInList = highScores[MAX_HIGH_SCORES - 1].score;
    return score > lowestScoreInList;
}

/**
 * Añade una nueva puntuación a la lista, la ordena y la recorta.
 * @param {object} newScoreEntry - El objeto de puntuación a añadir.
 */
function addHighScore(newScoreEntry) {
    const highScores = getHighScores();
    highScores.push(newScoreEntry);

    // Ordena la lista según los criterios definidos
    highScores.sort((a, b) => {
        // 1. Por Puntuación (descendente)
        if (b.score !== a.score) return b.score - a.score;
        // 2. Por Kills (descendente)
        if (b.kills !== a.kills) return b.kills - a.kills;
        // 3. Por Nivel (descendente)
        if (b.level !== a.level) return b.level - a.level;
        // 4. Por Dificultad (descendente)
        if (b.difficulty !== a.difficulty) return b.difficulty - a.difficulty;
        // 5. Por Nombre (alfabético, ascendente)
        return a.name.localeCompare(b.name);
    });

    // Recorta la lista para mantener solo los 20 mejores
    const trimmedScores = highScores.slice(0, MAX_HIGH_SCORES);
